        ...
        in
        {
          handy = pkgs.rustPlatform.buildRustPackage {
            ...
            postPatch = ''
              ...
              # Remove duplicate crates-io registry definition introduced by extraRegistries
              if [ -f "$cargoDepsCopy/.cargo/config.toml" ]; then
                sed -i '/\[source\."https:\/\/github\.com\/rust-lang\/crates\.io-index"\]/,+2d' "$cargoDepsCopy/.cargo/config.toml"
              fi
              if [ -f "src-tauri/.cargo/config.toml" ]; then
                sed -i '/\[source\."https:\/\/github\.com\/rust-lang\/crates\.io-index"\]/,+2d' "src-tauri/.cargo/config.toml"
              fi
